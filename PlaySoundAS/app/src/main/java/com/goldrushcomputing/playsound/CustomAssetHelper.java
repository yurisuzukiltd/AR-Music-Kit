...
public class CustomAssetHelper {
	...
	public CustomAssetHelper(AssetManager am) {
		manager = am;
	}
	...
	public void cacheAssetFolder(Context ctx, String assetBasePath) {
		...
		//..File cacheFolder = new File(ctx.getExternalCacheDir().getAbsolutePath() + "/" + assetBasePath);
		...
		if (reCache) {
			...
			//..List<AssetFileTransfer> transfers = copyAssetFolder(assetBasePath, ctx.getExternalCacheDir().getAbsolutePath()); // Recreate it.
